resource "aws_iam_role" "bigip-failover-extension-iam-role" {
  name = "${var.prefix}-${var.tenant_name}-bigip-failover-extension-iam-role"
  assume_role_policy = file("${path.module}/templates/bigip-failover-extension-iam-role-assume-role.json")
  tags = {
    name = "${var.prefix}-${var.tenant_name}-f5_cloud_failover_iam_role"
  }
}

resource "aws_iam_policy" "bigip-failover-extension-iam-policy" {
  name        = "${var.prefix}-${var.tenant_name}-bigip-failover-extension-iam-policy"
  description = "for bigip cloud failover extension"
  policy      = file("${path.module}/templates/bigip-failover-extension-iam-policy.json")
}
...
resource "aws_iam_policy_attachment" "bigip-failover-extension-iam-policy-attach" {
  ...
  name       = "${var.prefix}-${var.tenant_name}-bigip-failover-extension-iam-policy-attach"
...
resource "aws_iam_instance_profile" "bigip-failover-extension-iam-instance-profile" {
  ...
  name = "${var.prefix}-${var.tenant_name}-bigip-failover-extension-iam-instance-profile"
