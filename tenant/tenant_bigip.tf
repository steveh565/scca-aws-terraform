...
resource "null_resource" "az1_tenant_external_secondary_ips" {
  depends_on = [aws_network_interface.az1_tenant_external]
...
resource "null_resource" "az1_tenant_internal_secondary_ips" {
  depends_on = [aws_network_interface.az1_tenant_internal]
  # Use the "aws ec2 assign-private-ip-addresses" command to add secondary addresses to an existing network interface 
  #    -> Workaround for bug: https://github.com/terraform-providers/terraform-provider-aws/issues/10674
  provisioner "local-exec" {
    command = <<-EOF
...
resource "aws_eip" "eip_az1_tenant_external" {
  depends_on                = [aws_network_interface.az1_tenant_external, null_resource.az1_tenant_external_secondary_ips, aws_internet_gateway.tenantGw]
  vpc                       = true
...
resource "null_resource" "az2_tenant_external_secondary_ips" {
  depends_on = [aws_network_interface.az2_tenant_external]
  # Use the "aws ec2 assign-private-ip-addresses" command to add secondary addresses to an existing network interface 
  #    -> Workaround for bug: https://github.com/terraform-providers/terraform-provider-aws/issues/10674
  provisioner "local-exec" {
    command = <<-EOF
...
resource "null_resource" "az2_tenant_internal_secondary_ips" {
  depends_on = [aws_network_interface.az2_tenant_internal]
  # Use the "aws ec2 assign-private-ip-addresses" command to add secondary addresses to an existing network interface 
  #    -> Workaround for bug: https://github.com/terraform-providers/terraform-provider-aws/issues/10674
  provisioner "local-exec" {
    command = <<-EOF
...
resource "aws_eip" "eip_az2_tenant_external" {
  depends_on                = [aws_network_interface.az2_tenant_external, null_resource.az2_tenant_external_secondary_ips, aws_internet_gateway.tenantGw]
  vpc                       = true
